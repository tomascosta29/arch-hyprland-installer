...
class SettingsDialog(Adw.Window):
    def __init__(self, parent):
        ...
        dir_row.append(self.dir_entry); dir_row.append(dir_browse)
        ...
        self.pattern_entry.set_text(self.config.get("naming_pattern", DEFAULT_CONFIG["naming_pattern"]))
        self.pattern_entry.set_hexpand(True)
        pattern_box.append(self.pattern_entry)
        pattern_help = Gtk.Label(label="Use strftime format: %Y=year, %m=month, %d=day, %H=hour, %M=minute, %S=second")
        ...
        clipboard_row = Adw.ActionRow(title="Copy to Clipboard", subtitle="Automatically copy screenshot to clipboard")
        ...
        notification_row = Adw.ActionRow(title="Show Notification", subtitle="Display notification after capture")
        ...
        open_manager_row = Adw.ActionRow(title="Open Manager After Capture", subtitle="Launch manager after taking screenshot")
    ...
    def on_browse_dir(self, _):
        ...
        dialog.set_initial_file(Gio.File.new_for_path(os.path.expanduser(self.dir_entry.get_text())))
...
class BlinkerManagerWindow(Adw.ApplicationWindow):
    ...
    def build_ui(self):
        ...
        preview_frame = Gtk.Frame(); preview_frame.add_css_class("preview-pane")
        ...
        scroll_controller = Gtk.EventControllerScroll.new(Gtk.EventControllerScrollFlags.VERTICAL | Gtk.EventControllerScrollFlags.DISCRETE)
    ...
    def setup_monitor(self):
        ...
        if os.path.exists(SCREENSHOTS_DIR):
            gio_file = Gio.File.new_for_path(SCREENSHOTS_DIR)
    ...
    def reload(self):
        if not os.path.exists(SCREENSHOTS_DIR): self.files = []
        else:
            files = [os.path.join(SCREENSHOTS_DIR, f) for f in os.listdir(SCREENSHOTS_DIR) if f.lower().endswith((".png", ".jpg", ".jpeg", ".webp"))]
    ...
    def get_date_label(self, path):
        ...
        if diff.days < 7: return mtime.strftime("%A")
    ...
    def make_grid_item(self, path):
        ...
        img = Gtk.Image(); img.set_pixel_size(100)
        GLib.idle_add(self.load_row_thumb, path, img)
        
        frame = Gtk.Frame(); frame.add_css_class("grid-thumb-frame"); frame.set_child(img)
        item.set_child(frame)
        return item

    def apply_filter(self):
        query = self.search.get_text().strip()
        self.filtered = [f for f in self.files if not query or query.lower() in os.path.basename(f).lower()]
        ...
        while (child := self.grid_view.get_first_child()): self.grid_view.remove(child)
            
        visible_paths = set(self.filtered)
        stale_paths = set(self.row_cache.keys()) - visible_paths
        for path in stale_paths: del self.row_cache[path]
        ...
        for i, path in enumerate(self.filtered[:100]):
            ...
            if label != last_label:
                header_row = Gtk.ListBoxRow(); header_row.set_selectable(False)
                lbl = Gtk.Label(label=label); lbl.add_css_class("heading"); lbl.set_halign(Gtk.Align.START)
                lbl.set_margin_start(16); lbl.set_margin_top(12); lbl.set_margin_bottom(6)
                header_row.set_child(lbl); self.listbox.append(header_row)
                last_label = label

            if path not in self.row_cache: self.row_cache[path] = self.make_row(path, i + 1)
    ...
    def make_row(self, path, index):
        row = Gtk.ListBoxRow(); row._file_path = path
        box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10); box.add_css_class("history-row")
        row.set_child(box)
        thumb_img = Gtk.Image(); thumb_img.set_pixel_size(60)
        ...
        thumb_frame = Gtk.Frame(); thumb_frame.add_css_class("thumb-frame"); thumb_frame.set_child(thumb_img)
        box.append(thumb_frame)
        lbl = Gtk.Label(label=os.path.basename(path)); lbl.set_ellipsize(Pango.EllipsizeMode.END); box.append(lbl)
        return row
    ...
    def copy_current(self):
        ...
        if len(valid_rows) > 1:
            uris = [Gio.File.new_for_path(row._file_path).get_uri() for row in valid_rows]
            uri_list = "\n".join(uris)
            subprocess.run(["wl-copy", "--type", "text/uri-list"], input=uri_list.encode(), check=False)
        else:
            path = valid_rows[0]._file_path
            mime = "image/png" if path.lower().endswith(".png") else "image/jpeg"
            subprocess.run(["wl-copy", "--type", mime], stdin=open(path, "rb"), check=False)

    def on_ocr_clicked(self, _):
        if not self.current_file: return
        def run_ocr():
            try:
                proc = subprocess.run(["tesseract", self.current_file, "stdout"], capture_output=True, text=True)
                if proc.stdout.strip(): subprocess.run(["wl-copy"], input=proc.stdout.encode())
            except: pass
        import threading; threading.Thread(target=run_ocr).start()
    ...
    def on_pin_clicked(self, _):
        ...
        for row in valid_rows:
            path = row._file_path
            if path in self.pinned: self.pinned.remove(path)
            else: self.pinned.add(path)
        self.save_pins(); self.reload()

    def load_pins(self):
        try:
            with open(PINS_FILE, "r") as f: return set(l.strip() for l in f)
        except: return set()

    def save_pins(self):
        os.makedirs(CONFIG_DIR, exist_ok=True)
        with open(PINS_FILE, "w") as f:
            for p in self.pinned: f.write(f"{p}\n")
        ...
        try:
            for row in valid_rows:
                ...
                if os.path.exists(path):
                    ...
                    file.trash(None)
                    if path in self.row_cache: del self.row_cache[path]
                    if path in self.thumb_cache: del self.thumb_cache[path]
            self.reload()
        except: pass
    ...
    def on_move_clicked(self, _):
        ...
        dialog = Gtk.FileDialog()
        dialog.select_folder(self, None, lambda d, r: self.on_folder_selected(d, r, valid_rows))

    def on_folder_selected(self, dialog, result, selected_rows):
        try:
            folder = dialog.select_folder_finish(result)
            if folder:
