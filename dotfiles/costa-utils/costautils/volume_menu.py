...
class VolumeWindow(Adw.ApplicationWindow):
    ...
    def build_ui(self):
        ...
        out_lbl.set_hexpand(True); out_lbl.set_halign(Gtk.Align.START)
        self.out_vol_val = Gtk.Label(label="0%")
        self.out_vol_val.add_css_class("dim-label")
        out_lbl_box.append(out_lbl); out_lbl_box.append(self.out_vol_val)
        ...
        in_lbl.set_hexpand(True); in_lbl.set_halign(Gtk.Align.START)
        self.in_vol_val = Gtk.Label(label="0%")
        self.in_vol_val.add_css_class("dim-label")
        in_lbl_box.append(in_lbl); in_lbl_box.append(self.in_vol_val)
    ...
    def refresh_audio_devices(self):
        def worker():
            try:
                # Sinks
                s_proc = subprocess.run(["pactl", "-f", "json", "list", "sinks"], capture_output=True, text=True)
                sinks = json.loads(s_proc.stdout) if s_proc.returncode == 0 else []
                if isinstance(sinks, dict): sinks = [sinks]
                
                # Sources
                r_proc = subprocess.run(["pactl", "-f", "json", "list", "sources"], capture_output=True, text=True)
                ...
                d_sink_proc = subprocess.run(["pactl", "get-default-sink"], capture_output=True, text=True)
                def_sink = d_sink_proc.stdout.strip()
                
                d_source_proc = subprocess.run(["pactl", "get-default-source"], capture_output=True, text=True)
    ...
    def update_devices_ui(self, sinks, sources, def_sink, def_source):
        ...
        for src in sources:
            ...
            if active: active_source_obj = src
        ...
        try:
            if active_sink_obj:
                ...
                self.out_mute_btn.set_icon_name("audio-volume-muted-symbolic" if active_sink_obj["mute"] else "audio-volume-high-symbolic")
            ...
            if active_source_obj:
                ...
                self.in_mute_btn.set_icon_name("audio-input-microphone-muted-symbolic" if active_source_obj["mute"] else "audio-input-microphone-symbolic")
    ...
    def make_device_row(self, device, active):
        ...
        icon_name = "audio-card-symbolic" if "sink" in device.get("media.class", "").lower() else "audio-input-microphone-symbolic"
        icon = Gtk.Image.new_from_icon_name(icon_name)
        if active: icon.add_css_class("accent-icon")
        box.append(icon)
        
        lbl = Gtk.Label(label=device.get("description", device.get("name", "Unknown")))
        lbl.set_hexpand(True); lbl.set_halign(Gtk.Align.START)
        if active: lbl.add_css_class("bold-label")
        box.append(lbl)
    ...
    def on_input_slider_changed(self, scale):
        ...
        subprocess.run(["wpctl", "set-volume", "@DEFAULT_AUDIO_SOURCE@", f"{val}%"], stdout=subprocess.DEVNULL)
