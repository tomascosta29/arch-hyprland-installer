...
PINS_FILE = os.path.join(STATE_DIR, "pins")
STATE_FILE = os.path.join(STATE_DIR, "state.json")
LEGACY_DIR = os.path.expanduser("~/.config/clipper")


def clipboard_mime_type(data):
...
class ClipWindow(Adw.ApplicationWindow):
    ...
    def load_state(self):
        ...
        legacy_path = os.path.join(LEGACY_DIR, "state.json")
        if not os.path.exists(state_path) and os.path.exists(legacy_path):
    ...
    def build_ui(self):
        ...
        img_box = Gtk.ScrolledWindow()
        img_box.set_child(self.preview_image)
        ...
        txt_box = Gtk.ScrolledWindow()
        txt_box.set_child(self.preview_text)
        ...
        self.color_preview_box.set_halign(Gtk.Align.CENTER)
        self.color_swatch = Gtk.Frame()
        self.color_swatch.set_size_request(200, 200)
        self.color_swatch.add_css_class("color-swatch")
        self.color_preview_box.append(self.color_swatch)
        self.color_label = Gtk.Label()
        self.color_label.add_css_class("color-label")
        self.color_preview_box.append(self.color_label)

        self.empty_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=12)
        self.empty_box.set_valign(Gtk.Align.CENTER)
        self.empty_icon = Gtk.Image.new_from_icon_name("edit-find-symbolic")
        self.empty_icon.set_pixel_size(64)
        self.empty_icon.add_css_class("dim-label")
        self.empty_box.append(self.empty_icon)
        self.preview_label = Gtk.Label(label="Select a clipboard item")
        self.preview_label.add_css_class("dim-label")
        self.empty_box.append(self.preview_label)
    ...
    def load_pins(self):
        pins_path = PINS_FILE
        legacy_path = os.path.join(LEGACY_DIR, "pins")
        if not os.path.exists(pins_path) and os.path.exists(legacy_path):
            pins_path = legacy_path
    ...
    def apply_filter(self):
        ...
        if f_text:
            filtered_list = [line for line in self.lines if "[[ binary data" not in line]
        elif f_imgs:
            filtered_list = [line for line in self.lines if "[[ binary data" in line]

        if query:
            self.filtered = [line for line in filtered_list if self.fuzzy_match(query, line)]
        else:
            self.filtered = list(filtered_list)

        self.filtered.sort(key=lambda line: line.split("\t", 1)[0] not in self.pinned)

        while row := self.listbox.get_first_child():
            self.listbox.remove(row)
        if not self.filtered:
            self.preview_label.set_label("No results found")
            self.preview_stack.set_visible_child_name("empty")
        else:
            if self.preview_label.get_label() == "No results found":
                self.preview_label.set_label("Select a clipboard item")
    ...
    def make_row(self, line, index):
        ...
        if "[[ binary data" in line:
            icon_name = (
                ...
                if any(x in line.lower() for x in ["png", "jpg", "jpeg", "webp"])
                else "package-x-generic-symbolic"
            )

        thumb = Gtk.Image.new_from_icon_name(icon_name)
        thumb.set_pixel_size(26)
        row._thumb = thumb
        ...
        content_box.append(top_line)

        if index <= 9:
            badge = Gtk.Label(label=f"ALT+{index}")
            badge.add_css_class("shortcut-badge")
            top_line.append(badge)
        if line_id in self.pinned:
            star = Gtk.Image.new_from_icon_name("emblem-favorite-symbolic")
            star.add_css_class("pinned-badge")
            top_line.append(star)
    ...
    def on_row_selected(self, _, row):
        ...
        if hasattr(self, "_select_timeout_id") and self._select_timeout_id:
            GLib.source_remove(self._select_timeout_id)
    ...
    def show_preview_from_data(self, data):
        ...
        if texture:
            ...
            if color_match:
                ...
                try:
                    json.loads(text)
                    self.json_btn.set_visible(True)
                    self.metadata_type.set_label("JSON")
                except Exception:
                    self.json_btn.set_visible(False)
    ...
    def on_edit_toggled(self, btn):
        ...
        if editable:
            self.preview_text.add_css_class("editable")
        else:
            self.preview_text.remove_css_class("editable")

    def on_json_clicked(self, _):
        try:
            obj = json.loads(self._current_text_data)
            pretty = json.dumps(obj, indent=2)
            self._current_text_data = pretty
            self.preview_text.get_buffer().set_text(pretty, -1)
        except Exception:
            pass
    ...
    def on_wipe_clicked(self, *args):
        dialog = Gtk.AlertDialog(
            message="Clear all clipboard history?", detail="Pinned items will be preserved."
        )
        dialog.set_buttons(["Cancel", "Wipe All"])
        dialog.set_default_button(1)
        dialog.choose(self, None, self.on_wipe_confirmed)
    ...
    def on_copy_clicked(self, _):
        self.copy_current_and_close(close=False)

    def on_row_activated(self, _, row):
        self.current_line = row._clip_line
        self.copy_current_and_close(close=True)
    ...
    def update_row_thumb_from_data(self, line, data):
        ...
        while row:
            if getattr(row, "_clip_line", None) == line:
                row._thumb.set_from_paintable(texture)
                break
    ...
    def show_preview_error(self, message):
        self.preview_label.set_label(message)
        self.preview_stack.set_visible_child_name("empty")
    ...
    def on_key_pressed(self, _, keyval, keycode, state):
        ...
        if name in ("Escape", "<Control>w", "<Control>q"):
            if self.listbox.get_selected_rows():
                self.listbox.unselect_all()
            else:
                self.close()
            return True
